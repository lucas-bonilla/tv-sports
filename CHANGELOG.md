...
- **Missing World Cup matches (whole days were dropped)**: the free TheSportsDB tier truncates `eventsseason.php`/`eventsnextleague.php`/`eventspastleague.php` to a handful of events, so fixtures like Ivory Coast–Ecuador and Belgium–Egypt never appeared. Fixtures are now fetched per UTC day via `eventsday.php?d=<date>&l=<league>`, which returns the full daily slate, walked across the needed date range and deduped by event id (best-effort per day)
